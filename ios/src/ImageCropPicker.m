...
RCT_EXPORT_METHOD(openCamera:(NSDictionary *)options
    ...
    [self checkCameraPermissions:^(BOOL granted) {
        ...
        if([[self.options objectForKey:@"isRecord"] boolValue]){
...
RCT_EXPORT_METHOD(convertHEIC:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject){
     [self getPath:[options objectForKey:@"path"]
                             definition :[[options objectForKey:@"compressImageQuality"] floatValue]
                            resolver:resolve
                                rejecter:reject
                            ];
    
//    if(result) resolve(result);
//    else reject(@"error",@"heic result is nil",nil);
}

-(void)getPath: (NSString*) uri
    ...
    if ([allTrim(uri) length] != 0) {
        ...
        NSLog(@"heicImageFilePath ext=========%@====%d",ext,[ext localizedCaseInsensitiveContainsString:@"HEIC"]);
        if (definition <= 0 ) {
            definition = 1;
        }
        if ([ext localizedCaseInsensitiveContainsString:@"HEIC"]) {
           
        [self getPathFromUri:uri completionHandler:^(NSString *path, ALAssetRepresentation *asset) {
            ...
            if(asset != nil){
                ...
                if(err != nil){
                    reject(@"error",@"heic result is nil",nil);
                    free(buffer);
                    return;
                }
            ...
            NSData *jpgImageData = UIImageJPEGRepresentation(image, definition);
            NSString * newJpgPath =[self persistFile:jpgImageData ];
          
            if(newJpgPath){
                resolve(@{ @"success": @YES, @"path": newJpgPath});
            }else{
                reject(@"error",@"heic result is nil",nil);
            }
...
- (void) getPathFromUri:(NSString *)uri
      completionHandler:(void(^)(NSString * path, ALAssetRepresentation *asset)) onComplete{
    if([uri hasPrefix:@"assets-library://"]){
        ...
        onComplete([self getPathOfAsset:uri], nil);
    }
}

-(NSString *) getPathOfAsset:(NSString *)assetURI{
        NSString *assetURItag = [[NSBundle mainBundle] pathForResource: [assetURI stringByDeletingPathExtension]
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    if([[self.options objectForKey:@"isRecord"] boolValue]){
        NSURL * mediaUrl = [info objectForKey: UIImagePickerControllerMediaURL];
         NSLog(@"UIImagePickerController=====UIImagePickerControllerMediaURL=========%@",mediaUrl);
            ...
            if(mediaUrl){
                [self showActivityIndicator:^(UIActivityIndicatorView *indicatorView, UIView *overlayView) {
                        ...
                        [library writeVideoAtPathToSavedPhotosAlbum:mediaUrl completionBlock:^(NSURL *assetURL, NSError *error) {
                            if (!error) {
//                                NSLog(@"Save video succeed.====assetURL=%@",assetURL);
                                    if (assetURL) {
                                            PHAsset *capturedAsset = [PHAsset fetchAssetsWithALAssetURLs:@[assetURL] options:nil].lastObject;
                                            NSLog(@"Save video succeed.====capturedAsset=%@",capturedAsset);
                                            if(capturedAsset){
...
-(void)compressImageWithOpts:(NSMutableDictionary *)info{
...
- (void)mkResolveMediaData:(void (^)(NSDictionary *))completion forAsset:(PHAsset *)forAsset outputURL:(NSURL *)outputURL sourceURL:(NSURL *)sourceURL thumbinilPath:(NSString *)thumbinilPath {
    NSDictionary *opts = [NSDictionary dictionaryWithObject:@(NO) forKey:AVURLAssetPreferPreciseDurationAndTimingKey];
//    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:outputURL options:opts];
    AVURLAsset *orginAsset = [AVURLAsset URLAssetWithURL:sourceURL options:opts];
    AVAsset *compressedAsset = [AVAsset assetWithURL:outputURL];
    
    NSUInteger second = 0;
    second = (NSUInteger)orginAsset.duration.value/orginAsset.duration.timescale;
    
    AVAssetTrack *track = [[compressedAsset tracksWithMediaType:AVMediaTypeVideo] firstObject];
    
    UIImage *thumbnailImg=[self loadThumbNail :outputURL];
    NSString *thumbnailImgPath=[self saveThumailToLocal:thumbnailImg videoName:thumbinilPath];
    
    NSLog(@"thumbnailImgPath======outputURL==%@==second==%ld",outputURL,second);
    ...
    completion([self createAttachmentResponse:[outputURL absoluteString]
                         ...
                         withModificationDate:forAsset.modificationDate
                           withVideoThumbnail:thumbnailImgPath
                            withVideoDuration:[NSNumber numberWithInteger:second]
                ]);
}
...
- (void) getVideoAsset:(PHAsset*)forAsset completion:(void (^)(NSDictionary* image))completion {
    ...
    [manager
     ...
     resultHandler:^(AVAsset * asset, AVAudioMix * audioMix,NSDictionary *info) {
         ...
         NSString  * thumbinilPath = [filePath stringByAppendingString:@".png"];
         filePath = [filePath stringByAppendingString:@".mp4"];
         NSURL *outputURL = [NSURL fileURLWithPath:filePath];
         AVAssetTrack *originTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] firstObject];
         NSLog(@"originTrack.naturalSize.width=%f==height=%f",originTrack.naturalSize.width,originTrack.naturalSize.height);
         
         if(originTrack.naturalSize.width>originTrack.naturalSize.height){
             if(originTrack.naturalSize.height<=1080){
                 [self.options setValue:@"Passthrough" forKey:@"compressVideoPreset"];
             }
             
         }else{
             if(originTrack.naturalSize.width<=1080){
                 [self.options setValue:@"Passthrough" forKey:@"compressVideoPreset"];
             }
         ...
         [self.compression compressVideo:sourceURL outputURL:outputURL withOptions:self.options handler:^(AVAssetExportSession *exportSession) {
             if (exportSession.status == AVAssetExportSessionStatusCompleted) {
                 [self mkResolveMediaData:completion forAsset:forAsset outputURL:outputURL sourceURL:sourceURL thumbinilPath:thumbinilPath];
             } else {
                 completion(nil);
             }
         }];
...
-(NSString *)saveThumailToLocal:(UIImage *)image videoName:(NSString *)videoName{
    NSData *imageData = UIImagePNGRepresentation(image);
//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *documentsDirectory = [paths objectAtIndex:0];
    
//    NSString *imagePath =[documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.png",videoName]];
    
    NSLog(@"saveThumailToLocal ,pre writing to file==%@",videoName);
    if (![imageData writeToFile:videoName atomically:NO])
    {
        NSLog(@"saveThumailToLocal ,Failed to cache image data to disk");
        return @"";
    }
    else
    {
        NSLog(@"saveThumailToLocal  ,the cachedImagedPath is %@",videoName);
        return videoName;
    }
}
...
- (NSDictionary*) createAttachmentResponse:(NSString*)filePath withExif:(NSDictionary*) exif withSourceURL:(NSString*)sourceURL withLocalIdentifier:(NSString*)localIdentifier withFilename:(NSString*)filename withWidth:(NSNumber*)width withHeight:(NSNumber*)height withMime:(NSString*)mime withSize:(NSNumber*)size withData:(NSString*)data withRect:(CGRect)cropRect withCreationDate:(NSDate*)creationDate withModificationDate:(NSDate*)modificationDate withVideoThumbnail:(NSString*)videoThumbnail withVideoDuration:(NSNumber*)videoDuration{
    ...
    return @{
             @"path": (filePath && ![filePath isEqualToString:(@"")]) ? filePath : @"",
             ...
             @"data": (data) ? data : @"",
             @"exif": (exif) ? exif : @"",
             @"cropRect": CGRectIsNull(cropRect) ? @"" : [ImageCropPicker cgRectToDictionary:cropRect],
             @"creationDate": (creationDate) ? [NSString stringWithFormat:@"%.0f", [creationDate timeIntervalSince1970]] : @"",
             @"modificationDate": (modificationDate) ? [NSString stringWithFormat:@"%.0f", [modificationDate timeIntervalSince1970]] : @"",
             @"videoThumbnail": (videoThumbnail) ? videoThumbnail : @"",
             @"videoDuration": videoDuration&&[videoDuration intValue]>0 ? videoDuration : [NSNumber numberWithInt:0],
             };
}
                             ...
                             @autoreleasepool {
                                 ...
                                 if([[self.options objectForKey:@"writeTempFile"] boolValue]) {
                                     ...
                                     if (filePath == nil) {
                                        ...
                                        NSLog(@"QBImagePickerController======no==video=====filePath=%@",filePath);
                                 }
